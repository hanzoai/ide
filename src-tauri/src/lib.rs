// Hanzo — Tauri entry point.
// Boots Hanzo V2's window with the full Hanzo AI engine state and commands.
// All engine logic lives in the `hanzo_engine` crate (Hanzo AI/src-tauri).

// Hanzo AI crate provides engine + indexer

use hanzo_engine::commands;
use tauri::Manager;
use tauri::WebviewUrl;

const HANZO_IDENTITY: &str = r#"# Hanzo Agent Identity

You are the coding agent inside Hanzo, a native desktop IDE built with Rust and Tauri.
...
You have access to all tools the user has authorized. Call tools directly without conversational confirmation; the user controls approval through Hanzo's mode toggle (Ask / Auto / Yolo) and a banner will surface when the engine needs their input. Treat that banner as out of band: do not stall the agent loop waiting for it. If a tool call fails, handle the error and try a different approach.
...
/// Open a new Hanzo window (multi-window support)
#[tauri::command]
async fn open_new_window(app: tauri::AppHandle, folder_path: Option<String>) -> Result<String, String> {
    let window_id = format!("hanzo-{}", uuid::Uuid::new_v4().to_string().split('-').next().unwrap_or("0"));
    ...
    log::info!("[hanzo] opened new window: {}", window_id);
    Ok(window_id)
}
...
/// Open the Hanzo chat in a detached window. The detached window loads
...
/// `hanzo:chat:detached-state`. Engine events broadcast to all webviews
...
async fn open_chat_window(app: tauri::AppHandle) -> Result<String, String> {
    let window_label = "hanzo-chat-detached";
    ...
    // On macOS tie the chat window's lifecycle to the main Hanzo window
    ...
    log::info!("[hanzo] opened detached chat window: {}", window_label);
...
async fn close_chat_window(app: tauri::AppHandle) -> Result<(), String> {
    if let Some(window) = app.get_webview_window("hanzo-chat-detached") {
...
/// menu).
///
/// Accelerator policy: Hanzo runs a full VS Code workbench in the webview, and
...
fn build_app_menu(
    ...
    let app_menu = Submenu::with_items(
        ...
        true,
        &[
            &PredefinedMenuItem::about(handle, Some("About Hanzo"), Some(AboutMetadata::default()))?,
    ...
    let help_menu = Submenu::with_items(
        ...
        true,
        &[
            &MenuItem::with_id(handle, "help.docs", "Hanzo Documentation", true, None::<&str>)?,
        ],
    )?;
...
///
/// Privacy-first: Hanzo never phones home. Panics are written to a local
...
fn install_panic_hook() {
    ...
    std::panic::set_hook(Box::new(move |info| {
        ...
        let dir = hanzo_engine::engine::paths::data_dir();
...
pub fn run() {
    ...
    let engine_state = hanzo_engine::engine::state::EngineState::new()
        .expect("Failed to initialize Hanzo AI engine");

    // A chat with no provider is an IDE that does not work, and which account
    // this machine is signed in to is not something the window needs to be
    // asked about — take it from the CLI at startup.
    if let Err(e) = hanzo_engine::engine::cli::adopt_account(&engine_state) {
        log::warn!("[hanzo] could not adopt the signed-in account: {e}");
    }

    // Initialize cognitive event bus (required before engram/working-memory calls).
    hanzo_engine::engine::engram::cognitive_event::init();
    ...
    let mut builder = tauri::Builder::default()
        ...
        .manage(engine_state)
        .manage(hanzo_shell::terminal::TerminalState::new())
        .manage(hanzo_shell::watcher::WatcherState::new())
        .manage(hanzo_shell::lsp::LspState::new())
        .manage(hanzo_shell::dap::DapState::new())
        .manage(hanzo_shell::extension_host::ExtHostState::new())
        // Register Hanzo's IDE tool executor with the Hanzo AI engine
        .manage(Box::new(hanzo_ai::engine::HanzoToolExecutor) as Box<dyn hanzo_engine::engine::tools::ExternalToolExecutor>)
        // Hanzo ↔ Hanzo AI bridge: custom provider factory (ClaudeCode CLI)
        .manage(Box::new(hanzo_bridge::HanzoProviderFactory) as Box<dyn hanzo_engine::atoms::traits::ProviderFactory>)
        // Hanzo ↔ Hanzo AI bridge: tool assembler — controls which tools the model sees
        // Removes individual file tools (ide_read_file, ide_write_file, etc.)
        // forcing all file work through execute_code sandbox
        .manage(Box::new(hanzo_bridge::HanzoToolAssembler) as Box<dyn hanzo_engine::atoms::traits::ToolAssembler>)
        // Frontend bridge state (for tools that query Monaco editor)
        .manage(hanzo_ai::engine::frontend_bridge::FrontendBridgeState::new())
        // Codebase indexer state
        .manage(hanzo_ai::indexer::IndexerState {
            ...
            tauri_plugin_log::Builder::new()
                .target(tauri_plugin_log::Target::new(
                    tauri_plugin_log::TargetKind::LogDir {
                        file_name: Some("hanzo".into()),
                    },
                ))
                .max_file_size(5_000_000)
                .level(log::LevelFilter::Info)
                .level_for("hanzo_temp", log::LevelFilter::Debug)
                ...
                .level_for("hanzo_engine::engine::agent_loop::helpers", log::LevelFilter::Warn)
                .level_for("hanzo_engine::engine::binary_ipc", log::LevelFilter::Warn)
                .level_for("hanzo_engine::engine::engram::context_continuity", log::LevelFilter::Warn)
                .level_for("hanzo_engine::engine::engram::context_builder", log::LevelFilter::Warn)
                .level_for("hanzo_engine::engine::http", log::LevelFilter::Warn)

                // Per-message noise (fires every chat send)
                .level_for("hanzo_engine::engine::chat", log::LevelFilter::Warn)
                .level_for("hanzo_engine::engine::telemetry", log::LevelFilter::Warn)
                .level_for("hanzo_engine::engine::memory::embedding", log::LevelFilter::Warn)
                .level_for("hanzo_engine::engine::engram::encryption", log::LevelFilter::Warn)
                .level_for("hanzo_engine::engine::engram::graph", log::LevelFilter::Warn)
                .level_for("hanzo_engine::engine::engram::bridge", log::LevelFilter::Warn)
                .level_for("hanzo_engine::engine::engram::cognitive_state", log::LevelFilter::Warn)
                .level_for("hanzo_engine::engine::sessions::agent_files", log::LevelFilter::Warn)

                // Provider internals (thinking budget, request signing, cache stats)
                .level_for("hanzo_engine::engine::providers", log::LevelFilter::Warn)

                // Pricing / cost tracking (fake estimates, not real API costs)
                .level_for("hanzo_engine::engine::pricing", log::LevelFilter::Warn)

                // Skill library seeding (startup noise)
                .level_for("hanzo_engine::engine::engram::skill_library", log::LevelFilter::Warn)
                .level_for("hanzo_engine::engine::skills", log::LevelFilter::Warn)

                // MCP server lifecycle (spawning, connecting, tool counts)
                .level_for("hanzo_engine::engine::mcp::client", log::LevelFilter::Warn)
                .level_for("hanzo_engine::engine::mcp::transport", log::LevelFilter::Warn)

                // Extension host / bridge (adapter scan, registration)
                .level_for("hanzo_shell::extension_host", log::LevelFilter::Warn)

                // File watcher (watch/unwatch every folder open)
                .level_for("hanzo_shell::watcher", log::LevelFilter::Warn)

                // Terminal spawn/kill
                .level_for("hanzo_shell::terminal", log::LevelFilter::Warn)

                // Indexer (workspace open, cache load)
                .level_for("hanzo_ai::indexer", log::LevelFilter::Warn)
                .level_for("hanzo_ai::indexer::index", log::LevelFilter::Warn)

                // Bridge (procedural memory seeding)
                .level_for("hanzo_bridge", log::LevelFilter::Warn)
                .build(),
        )
        ...
        .setup(move |app| {
            // Logged here (not at builder time) so it lands in hanzo.log —
            // the log plugin only routes messages once the app is built.
            #[cfg(not(debug_assertions))]
            log::info!("[hanzo] serving frontend over http://localhost:{localhost_port}");

            // If no account is configured, look for an engine already running
            // on this machine. Off the startup path: a machine with none must
            // not wait for three ports to time out before it gets a window.
            {
                let handle = app.handle().clone();
                tauri::async_runtime::spawn(async move {
                    let state = handle.state::<hanzo_engine::engine::state::EngineState>();
                    match hanzo_engine::engine::engines::adopt_local(&state).await {
                        Ok(true) => log::info!("[hanzo] using a local engine"),
                        Ok(false) => {}
                        Err(e) => log::warn!("[hanzo] local engine probe failed: {e}"),
                    }
                });
            }

            // Join the local ZAP bus so the rest of the toolchain can reach a
            // running editor rather than only its own window.
            {
                let handle = app.handle().clone();
                let workspace = app
                    .state::<hanzo_engine::engine::state::EngineState>()
                    .store
                    .get_config("user_workspace_path")
                    .ok()
                    .flatten();
                tauri::async_runtime::spawn(hanzo_shell::zap::serve(handle, workspace));
            }
            ...
            // devUrl flow. Settings mirror the previous tauri.conf window.
            {
                ...
                // The workbench draws its own title row, so the window frame
                // only needs to supply the traffic lights. Overlay floats them
                // on top of the web content and drops the native bar.
                #[cfg(target_os = "macos")]
                let win = win
            ...
            // Set Hanzo identity on the default agent
            {
                let state = app.state::<hanzo_engine::engine::state::EngineState>();
                let store = &state.store;
                if let Err(e) = store.set_agent_file("default", "IDENTITY.md", HANZO_IDENTITY) {
                    log::error!("[hanzo] Failed to seed IDENTITY.md: {}", e);
                }

                // Seed Hanzo procedural memories — teaches agent to use WASM/sandbox/AST
                hanzo_bridge::seed_hanzo_procedural_memories(store);
            }

            // Hanzo: unlimited tool rounds — security audits on large codebases
            // need to run uninterrupted. Loop detector still catches stuck agents.
            {
                ...
                cfg.daily_budget_usd = 0.0; // Disable — Hanzo has no budget cap
            }

            // Cron heartbeat removed — Hanzo doesn't use cron tasks.
            // Re-add if Hanzo ever needs scheduled background tasks.
            ...
            tauri::async_runtime::spawn(async move {
                ...
                idx_handle.listen("open-workspace", move |event| {
                    ...
                    if let Some(path) = payload.get("path").and_then(|p| p.as_str()) {
                        ...
                        tauri::async_runtime::spawn(async move {
                            use tauri::Manager;
                            if let Some(state) = h.try_state::<hanzo_ai::indexer::IndexerState>() {
                                log::info!("[indexer] Workspace opened — indexing {}", workspace);
                                hanzo_ai::indexer::run_full_index(&workspace, &state, &h).await;
                            }
                        });
                    }
                });
        ...
        // ── Hanzo commands + essential Hanzo AI commands ─────────────────────────
        .invoke_handler(tauri::generate_handler![
            // ── Hanzo Native ────────────────────────────────────────────
            hanzo_shell::terminal::terminal_spawn,
            hanzo_shell::terminal::terminal_write,
            hanzo_shell::terminal::terminal_resize,
            hanzo_shell::terminal::terminal_kill,
            hanzo_shell::watcher::fs_watch,
            hanzo_shell::watcher::fs_unwatch,
            hanzo_shell::git::git_status,
            hanzo_shell::git::git_diff,
            hanzo_shell::git::git_stage,
            hanzo_shell::git::git_stage_all,
            hanzo_shell::git::git_unstage,
            hanzo_shell::git::git_commit,
            hanzo_shell::git::git_log,
            hanzo_shell::git::git_branches,
            hanzo_shell::git::git_checkout,
            hanzo_shell::git::git_checkpoint_create,
            hanzo_shell::git::git_checkpoint_restore,
            hanzo_shell::search::search_files,
            hanzo_shell::search::search_file_list,
            hanzo_shell::lsp::lsp_start,
            hanzo_shell::lsp::lsp_send,
            hanzo_shell::lsp::lsp_stop,
            hanzo_shell::lsp::lsp_list,
            hanzo_shell::dap::dap_start,
            hanzo_shell::dap::dap_send,
            hanzo_shell::dap::dap_stop,
            hanzo_shell::remote::remote_connect,
            hanzo_shell::remote::remote_exec,
            hanzo_shell::remote::remote_read_file,
            hanzo_shell::remote::remote_write_file,
            hanzo_shell::remote::remote_list_dir,
            hanzo_shell::extension_host::ext_host_start,
            hanzo_shell::extension_host::ext_host_send,
            hanzo_shell::extension_host::ext_host_stop,
            hanzo_shell::extension_host::ext_host_status,
            hanzo_shell::extension_host::ext_host_log,
            // B63/B64: Open VSX installer pipeline replaces curl/unzip shell.
            hanzo_shell::extensions::ext_fetch_url_text,
            hanzo_shell::extensions::ext_download_url_to_path,
            hanzo_shell::extensions::ext_extract_vsix,
            hanzo_shell::extensions::ext_get_disabled,
            hanzo_shell::extensions::ext_set_disabled,
            hanzo_ai::engine::frontend_bridge::ide_tool_response,
            hanzo_ai::engine::frontend_bridge::ide_edit_review_response,
            open_new_window,
            open_chat_window,
            close_chat_window,
            get_target_platform,
            hanzo_shell::ide_mcp::ide_read_file,
            hanzo_shell::ide_mcp::ide_write_file,
            hanzo_shell::ide_mcp::ide_read_file_bytes,
            hanzo_shell::ide_mcp::ide_write_file_bytes,
            hanzo_shell::ide_mcp::ide_stat,
            hanzo_shell::ide_mcp::open_external,
            hanzo_shell::ide_mcp::ide_delete_file,
            hanzo_shell::ide_mcp::ide_list_dir,
            hanzo_shell::ide_mcp::ide_run_command,
            hanzo_shell::ide_mcp::ide_get_git_status,
            hanzo_shell::ide_mcp::ide_get_git_diff,
            hanzo_shell::ide_mcp::ide_search_text,
            hanzo_shell::ide_mcp::ide_apply_edit,
            hanzo_ai::indexer::context::ide_get_codebase_context,
            hanzo_ai::indexer::context::ide_search_semantic,
            hanzo_ai::indexer::trigger_reindex,
            hanzo_ai::indexer::index_external_path,
            hanzo_ai::indexer::get_index_status,
            // ── Hanzo AI: Chat & Sessions ────────────────────────────────
            ...
            // ── Hanzo AI: Sign-in (Hanzo IAM, via the CLI) ──────────────
            ...
            // showed they're load-bearing for the Memory view.
    ...
    builder
        ...
        .expect("error while running Hanzo");
}
