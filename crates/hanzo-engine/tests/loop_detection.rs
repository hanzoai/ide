...
use hanzo_engine::engine::chat::detect_response_loop;
use hanzo_engine::engine::types::{Message, MessageContent, Role};
use hanzo_engine::engine::util::safe_truncate;
...
use hanzo_engine::engine::chat::{detect_user_override, is_user_override_phrase};
...
fn override_detects_stop_commands() {
    ...
    assert!(is_user_override_phrase("hanzo stop"));
...
fn detect_user_override_injects_system_redirect() {
    let mut msgs = vec![
        ...
        msg(
            Role::User,
            "HANZO stop! Focus on my instructions not the tasks. I am in control",
        ),
    ];
