...
pub fn detect_constraints(provider: ProviderKind, model: &str) -> ConstraintConfig {
    match provider {
        ...
        // ── Claude Code CLI ─────────────────────────────────────────
        // Tool calls are parsed from text output — no schema enforcement.
        ProviderKind::ClaudeCode => ConstraintConfig {
        ...
        // ── Custom / External ─────────────────────────────────────────
        // Unknown or host-provided provider — no constraints, rely on parse + retry.
        ProviderKind::Custom | ProviderKind::External => ConstraintConfig {
            level: ConstraintLevel::None,
            strict_tools: false,
            json_format: false,
            explicit_tool_choice: false,
