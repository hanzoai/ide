...
pub async fn delegate_to_worker(
    ...
    // n8n tool group removed in Hanzo phase 1.
    ...
    let system_prompt = "You are the FOREMAN (Worker Agent) for Hanzo AI.\n\n\
...
async fn run_worker_loop(
    ...
    for round in 1..=WORKER_MAX_ROUNDS {
        ...
        let chunks = provider
            .chat_stream(messages, tools, model, Some(0.0), None, None)
            .await
...
async fn execute_worker_tool(
    ...
    let result = if let Some(r) = tools::fetch::execute(name, &args, app_handle).await {
        ...
        // tools::n8n was removed in Hanzo phase 1.
