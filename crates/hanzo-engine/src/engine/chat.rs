// Hanzo Engine — Chat Workflow Organism
//
...
/// IDE tool allowlist — stored in Tauri managed state by host apps (Hanzo).
/// When present, only tools in this list (+ MCP tools) are sent to the AI model.
pub struct IdeToolAllowlist(pub Vec<String>);
...
pub fn build_chat_tools(
    ...
    // Phase 1 of the Hanzo extraction: skill_tools() always returns an
    ...
    // Add external tools (Hanzo IDE tools, if registered)
    let ext_tools = ToolDefinition::external_tools(app_handle);
    if !ext_tools.is_empty() {
        info!("[engine] Adding {} external (IDE) tools", ext_tools.len());
    ...
    // Hanzo phase 1; engine::skills no longer exists.
    let is_skill_required = |_name: &str| false;

    // Check if an external tool executor (Hanzo) is registered
    let has_external = app_handle
        .try_state::<Box<dyn crate::engine::tools::ExternalToolExecutor>>()
        .is_some();
    let ext_tool_names: Vec<String> = if has_external {
        app_handle
            .try_state::<Box<dyn crate::engine::tools::ExternalToolExecutor>>()
            .map(|ext| ext.tool_definitions_dynamic(app_handle).iter().map(|t| t.function.name.clone()).collect())
            .unwrap_or_default()
    } else {
        vec![]
    };
    let is_ide_tool = |name: &str| ext_tool_names.iter().any(|n| n == name);
    ...
    // Host app tool filtering via ToolAssembler trait (preferred) or legacy IdeToolAllowlist.
    if let Some(assembler) = app_handle.try_state::<Box<dyn crate::atoms::traits::ToolAssembler>>() {
        let before = t.len();
        t = assembler.filter_tools(t);
        info!("[engine] ToolAssembler filter: {} → {} tools", before, t.len());
    } else if has_external {
        // Legacy path: IdeToolAllowlist (will be removed once Hanzo migrates to ToolAssembler)
        if let Some(allowlist) = app_handle.try_state::<IdeToolAllowlist>() {
            let before = t.len();
            t.retain(|tool| {
                let name = tool.function.name.as_str();
                allowlist.0.contains(&name.to_string()) || name.starts_with("mcp_") || name.starts_with("wasm_")
            });
            info!("[engine] IDE tool filter: {} → {} tools", before, t.len());
        }
    }
...
pub fn build_runtime_context(
    ...
    // The default agent's display name is "Hanzo" (set in frontend), but the
...
/// what Hanzo AI is, what it can do, and how to do it — without guessing.
...
/// code that integrates cleanly with the Hanzo AI codebase and the wider
...
/// IDE-specific platform prompt — tells the agent it's running inside Hanzo.
/// Loaded from `prompts/ide_platform.md` at compile time.
pub fn build_ide_platform_prompt() -> &'static str {
    include_str!("prompts/ide_platform.md")
}

/// IDE-specific coding guidelines — code generation rules for Hanzo.
/// Loaded from `prompts/ide_coding.md` at compile time.
pub fn build_ide_coding_guidelines() -> &'static str {
    include_str!("prompts/ide_coding.md")
}

/// Build the MCP/Foreman Protocol awareness block (Hanzo AI chatbot mode).
///
/// Only used in Hanzo AI sessions — Hanzo uses build_ide_mcp_prompt() instead.
/// Loaded from `prompts/foreman.md` at compile time.
pub fn build_foreman_awareness() -> &'static str {
    include_str!("prompts/foreman.md")
}

/// Build the IDE-specific MCP awareness block (Hanzo mode).
///
/// Replaces build_foreman_awareness() in Hanzo sessions. Hanzo agents don't
/// need n8n, Slack tools, or the Foreman worker model — they just need to know
/// how to call MCP tools via ctx.tool() inside execute_code scripts.
/// Loaded from `prompts/ide_mcp.md` at compile time.
pub fn build_ide_mcp_prompt() -> &'static str {
    include_str!("prompts/ide_mcp.md")
}
...
///   2. Platform awareness manifest (what Hanzo AI is + all capabilities)
...
pub fn compose_chat_system_prompt(
    ...
    // In Hanzo: use IDE-specific prompts INSTEAD of generic Hanzo AI platform.
    // The IDE prompt tells the agent it's a coding assistant, not a general chatbot.
    // The generic platform prompt (email, Slack, trading, etc.) is not relevant in an IDE.
    parts.push(build_ide_platform_prompt().to_string());
    parts.push(build_ide_coding_guidelines().to_string());
    // IDE-specific MCP note — ctx.tool() usage inside execute_code.
    // Replaces foreman.md (n8n/Slack/worker model) which is irrelevant in IDE mode.
    parts.push(build_ide_mcp_prompt().to_string());
    ...
    // Detect Hanzo mode: if IDE platform prompt is being used, we're in IDE mode
    let is_ide_mode = parts.iter().any(|p| p.contains("Hanzo") || p.contains("execute_code"));

    if is_ide_mode {
        // Hanzo mode — clean IDE-focused context
        parts.push(
            "## Memory\n\
            Your identity and preferences are loaded below. \
            Use `memory_store` to save important information about the project for future sessions. \
            Use `memory_search` to recall past knowledge.".to_string()
        );
    } else {
        // Hanzo AI mode — full soul file guidance
...
pub fn compose_chat_system_prompt_budgeted(
    ...
    let mut parts: Vec<String> = vec![
        build_ide_platform_prompt().to_string(),
        build_ide_coding_guidelines().to_string(),
        // IDE-specific MCP note instead of foreman.md — keeps n8n/Slack/worker
        // model instructions out of the Hanzo coding agent context.
        build_ide_mcp_prompt().to_string(),
        runtime_context,
    ];
    ...
    if is_ide_mode {
        parts.push(
            "## Memory\n\
            Use `memory_store` to save project knowledge. Use `memory_search` to recall.".to_string()
        );
    } else {
        let soul_hint = if core_context.is_some() {
            "Your core soul files (IDENTITY.md, SOUL.md, USER.md) are loaded below. \
...
pub fn is_user_override_phrase(lower: &str) -> bool {
    // ── Frontend STOP wrapper (deep-session redirect) ───────────────────
    // When the frontend sends a redirect after 3+ completed runs, it prefixes
    // the message with this marker. Detect it directly so the override path
    // fires even for natural-language messages that contain no keywords.
    if lower.contains("redirect — stop your current task") {
        return true;
    }
