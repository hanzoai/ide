// Hanzo Engine — Google Gemini Provider
...
impl GoogleProvider {
    ...
    fn sanitize_schema(val: &Value) -> Value {
        Self::sanitize_schema_inner(val, 0)
    }

    fn sanitize_schema_inner(val: &Value, depth: usize) -> Value {
        // Guard against stack overflow on deeply nested schemas (tokio threads have 2MB stacks)
        if depth > 32 {
            return val.clone();
        }
        match val {
            Value::Object(map) => {
                ...
                for (k, v) in map {
                    ...
                    clean.insert(k.clone(), Self::sanitize_schema_inner(v, depth + 1));
                }
            ...
            Value::Array(arr) => Value::Array(arr.iter().map(|v| Self::sanitize_schema_inner(v, depth + 1)).collect()),
            other => other.clone(),
        }
    }
    ...
    async fn chat_stream_inner(
        ...
        if !tools.is_empty() {
            ...
            // Apply tool_choice override — map to Google's mode format
