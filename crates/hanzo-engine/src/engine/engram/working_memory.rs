...
impl WorkingMemory {
    ...
    pub fn decay_priorities(&mut self, factor: f32) {
        for slot in &mut self.slots {
            let source_factor = match slot.source {
                ...
                WorkingMemorySource::ToolResult => factor.powf(1.3).min(0.80), // cap at 0.80 per call — after 5 sends ≈ 0.33, auto-evicts
            };
    ...
    /// Evict all ToolResult and Recall slots — call this on redirect so stale
    /// audit findings don't compete with the new user message.
    /// UserMention and SensoryBuffer slots are preserved (they represent
    /// things the user explicitly referenced or very recent exchanges).
    pub fn evict_stale_task_slots(&mut self) -> usize {
        let before = self.slots.len();
        self.slots.retain(|s| {
            !matches!(
                s.source,
                WorkingMemorySource::ToolResult | WorkingMemorySource::Recall
            )
        });
        let removed = before - self.slots.len();
        // Recompute token total after bulk removal
        self.current_tokens = self.slots.iter().map(|s| s.token_cost).sum();
        if removed > 0 {
            log::info!(
