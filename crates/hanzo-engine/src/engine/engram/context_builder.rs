        ...
        if let Some(wm) = self.working_memory {
            ...
            if !wm_text.is_empty() {
                ...
                sections.push(PromptSection {
                    ...
                    content: wm_text,
                    priority: 7, // same level as recalled_memories — stale WM no longer outlasts fresh recalled context (0=highest, 10=lowest)
                    tokens: wm_tokens,
                    fallback: None,
                });
            }
...
/// Maximum character length for tool result messages outside the recent window.
///
/// execute_code results can contain full file contents, cargo output, etc. —
/// easily 50–200k chars per call. After 40+ tool rounds, this fills the entire
/// context budget and leaves the model stuck replaying the last audit. Older
/// tool results are truncated to a summary; the model already processed them.
const MAX_OLD_TOOL_RESULT_CHARS: usize = 150;

/// Number of most-recent tool result messages to preserve verbatim.
///
/// The model needs the last few results to understand where it is. Everything
/// before this window gets truncated to MAX_OLD_TOOL_RESULT_CHARS.
const RECENT_TOOL_ROUNDS: usize = 4;

/// Trim conversation history to fit within a token budget.
///
/// Two-pass process:
/// 1. Compress: truncate old tool results to MAX_OLD_TOOL_RESULT_CHARS so that
///    deep audit sessions don't fill the entire context with stale file content.
///    The last RECENT_TOOL_ROUNDS tool messages are kept verbatim.
/// 2. Drop: remove oldest messages from the front until within budget.
///
...
fn trim_history(
    ...
    // Find indices of all tool messages, keep the last RECENT_TOOL_ROUNDS verbatim.
    ...
    let compress_cutoff = tool_indices
        .len()
        .saturating_sub(RECENT_TOOL_ROUNDS);
    let old_tool_indices: std::collections::HashSet<usize> = tool_indices
        .iter()
        .take(compress_cutoff)
        .copied()
        .collect();

    let compressed: Vec<(String, String)> = messages
        .iter()
        .enumerate()
        .map(|(i, (role, content))| {
            if old_tool_indices.contains(&i) && content.len() > MAX_OLD_TOOL_RESULT_CHARS {
                let truncated = &content[..MAX_OLD_TOOL_RESULT_CHARS.min(content.len())];
                // Snap to a char boundary
                let safe_end = (0..=truncated.len())
                    .rev()
                    .find(|&e| truncated.is_char_boundary(e))
                    .unwrap_or(0);
                (
                    role.clone(),
                    format!(
    ...
    // ── Pass 2: Drop oldest messages until within budget ─────────────────
    let per_message_overhead = 4; // role tokens + formatting
    let costs: Vec<usize> = compressed
        .iter()
        .map(|(role, content)| {
    ...
    if total <= budget {
        return (compressed, total, 0);
    }

    let mut start = 0;
    let mut running = total;
    while running > budget && start < compressed.len() {
        running -= costs[start];
        start += 1;
    }

    let kept = compressed[start..].to_vec();
