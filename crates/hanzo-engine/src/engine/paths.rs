// Hanzo Engine — Centralized path management
//
// All paths under the Hanzo data root are resolved through this module.
// Default root: `~/.hanzo/`.  Users can override via a redirect file
// at `~/.hanzo/storage.conf` (single line: the new root path).
//
...
/// Cached override for the data root, loaded from `~/.hanzo/storage.conf`.
/// `None` → use default `~/.hanzo/`.  `Some(path)` → user-configured root.
static DATA_ROOT_OVERRIDE: RwLock<Option<PathBuf>> = RwLock::new(None);

/// The fixed location of the redirect file (always `~/.hanzo/storage.conf`).
fn storage_conf_path() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".hanzo").join("storage.conf"))
}

/// Load the data root override from `~/.hanzo/storage.conf`.
...
pub fn load_data_root_from_conf() {
    ...
    log::info!("[paths] Using default data root (~/.hanzo/)");
}

/// Persist the data root override to `~/.hanzo/storage.conf`.
pub fn save_data_root_to_conf(path: Option<&str>) -> Result<(), String> {
    ...
    if let Some(parent) = conf.parent() {
        std::fs::create_dir_all(parent).map_err(|e| format!("Cannot create ~/.hanzo/: {}", e))?;
    }
    match path {
...
/// The default data root: `~/.hanzo/`
pub fn default_data_dir() -> PathBuf {
    let home = dirs::home_dir().unwrap_or_else(|| PathBuf::from("."));
    home.join(".hanzo")
}

/// The root data directory for all Hanzo engine data.
/// Defaults to `~/.hanzo/`, overridable via Settings → Storage.
...
pub fn agent_workspace_dir(agent_id: &str) -> PathBuf {
    data_dir().join("workspaces").join(agent_id)
}
...
pub fn browser_profile_dir(profile_id: &str) -> PathBuf {
    data_dir().join("browser-profiles").join(profile_id)
}
