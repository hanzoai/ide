// Hanzo Engine — Tool RAG Index (Intent-Stated Retrieval)
//
...
pub const CORE_TOOLS: &[&str] = &[
    ...
    "list_directory",
    "request_tools",
    // Web access — always-on. These MUST be core: in Hanzo `request_tools`
    // (the dynamic tool loader) is execution-blocked, so any non-core tool
    // is unreachable there. Hanzo's assembler allowlist already exposes
...
/// Delegates to the centralized tool_metadata registry.
pub fn tool_domain(name: &str) -> &'static str {
    crate::engine::tool_metadata::domain_str(name)
}
...
mod tests {
    ...
    fn core_tools_contains_essentials() {
        ...
        // Web access must be core: Hanzo blocks request_tools, so non-core
