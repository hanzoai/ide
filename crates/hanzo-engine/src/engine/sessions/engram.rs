...
impl SessionStore {
    ...
    /// Delete all episodic memories scoped to a specific agent (across all sessions).
    /// Also removes associated graph edges so the memory graph stays consistent.
    pub fn engram_delete_episodic_by_agent(&self, agent_id: &str) -> EngineResult<usize> {
        let conn = self.conn.lock();
        // Collect IDs first so we can clean up edges
        let mut stmt =
            conn.prepare("SELECT id FROM episodic_memories WHERE scope_agent_id = ?1")?;
        let ids: Vec<String> = stmt
        ...
        let deleted = conn.execute(
            "DELETE FROM episodic_memories WHERE scope_agent_id = ?1",
            params![agent_id],
        )?;
        Ok(deleted)
    }
