// Database schema and migrations for the Hanzo engine store.
...
pub(crate) fn run_migrations(conn: &Connection) -> EngineResult<()> {
    ...
    // ── Skill Vault: credentials, enabled state, custom instructions ─
    conn.execute_batch(
        "
    ...
    // Unified Signed Audit Log was removed in Hanzo phase 1 along with
