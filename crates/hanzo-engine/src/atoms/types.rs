...
pub struct ProviderConfig {
    ...
    /// Which models from this provider should appear in the model selector.
    /// If empty/None, all preset models for this provider kind are shown.
    #[serde(default)]
...
pub enum ProviderKind {
    ...
    AzureFoundry,
    /// Claude Code CLI — uses the `claude` subprocess with Max subscription.
    /// No API key needed. Spawns `claude -p --output-format stream-json`.
    #[serde(alias = "claude_code")]
    ClaudeCode,
    /// External provider — registered by host app (e.g. Hanzo) via ProviderFactory trait.
    /// The provider implementation is supplied at runtime, not compiled into Hanzo AI.
    External,
}
...
pub enum EngineEvent {
    ...
    /// Intermediate agent analysis between tool-call rounds — survives session crashes
    /// and gives the activity feed visibility into what the agent found/concluded mid-run.
    ...
    /// Agent surfaced its findings — paused at round boundary for discussion.
    /// Frontend should unlock chat, show summary, and offer a Resume button.
...
pub struct ChatRequest {
    ...
    /// Active workspace path on disk.  Used to locate HANZO_NOTES.md for
    /// per-session persistent working memory auto-injection in the agent loop.
    #[serde(default)]
    pub workspace_path: Option<String>,
    /// True when the user sends a message while a run is already streaming.
    /// Signals the context assembler to prune old tool-call history so the
